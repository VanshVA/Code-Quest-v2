...
function TextPage({ questions = [], competition = {} }) {
  ...
  useEffect(() => {
    ...
    const nonEmptyAnswers = Object.entries(answers).filter(([_, value]) => value.trim() !== '').length;
    setAnsweredQuestions(nonEmptyAnswers);
  }, [answers]);
      ...
      <Card elevation={3}>
        <CardContent>
          ...
          <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
            <Typography variant="caption" color={answers[question._id]?.length > 500 ? 'error' : 'text.secondary'}>
              {answers[question._id]?.length || 0} / 2000 characters
            </Typography>
          </Box>
        </CardContent>
      </Card>
      ...
      <AppBar position="static" color="default" elevation={2}>
        <Toolbar>
          <Typography variant="h6" sx={{ flexGrow: 1 }}>
            {competition.competitionName || 'Text Competition'}
          </Typography>
                ...
                <Grid container spacing={1}>
                  {questions.map((q, index) => (
                    <Grid item xs={3} sm={2} md={4} key={index}>
                      <Button
                        variant={answers[q._id] && answers[q._id].trim() !== '' ? "contained" : "outlined"}
                        color={answers[q._id] && answers[q._id].trim() !== '' ? "primary" : "inherit"}
