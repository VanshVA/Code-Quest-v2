...
const adminDashboardController = {
  ...
  getAllCompetitions: async (req, res) => {
    try {
      ...
      if (req.query.availability === 'active') {
        filter.competitionAvailableTiming = { $lte: now };
        filter.endTiming = { $gte: now };
      } else if (req.query.availability === 'upcoming') {
        filter.competitionAvailableTiming = { $gt: now };
      ...
      const formattedCompetitions = await Promise.all(competitions.map(async (comp) => {
        ...
        try {
          // Attempt to find creator (could be teacher or admin)
          const teacher = await Teacher.findById(comp.creatorId).select('teacherFirstName teacherLastName');
          if (teacher) {
        ...
        const now = new Date();
        const availableFrom = new Date(comp.competitionAvailableTiming || comp.startTiming || comp.lastSaved);
        ...
        return {
          ...
          competitionAvailableTiming: comp.competitionAvailableTiming,
  ...
  getCompetitionById: async (req, res) => {
    try {
      ...
      const availableFrom = new Date(competition.competitionAvailableTiming || competition.startTiming || competition.lastSaved);
  ...
  createCompetition: async (req, res) => {
    try {
      ...
      const newCompetition = new Competition({
        ...
        competitionAvailableTiming: competitionAvailableTiming || startTiming || '',
  ...
  updateCompetition: async (req, res) => {
    try {
      ...
      if (competitionAvailableTiming !== undefined) competition.competitionAvailableTiming = competitionAvailableTiming;
  ...
  toggleCompetitionStatus: async (req, res) => {
    try {
      ...
      // If setting to live, ensure startTiming and competitionAvailableTiming are set
      if (competition.isLive) {
        ...
        if (!competition.competitionAvailableTiming) {
          competition.competitionAvailableTiming = now;
        }
      }
  ...
  getCompetitionStats: async (req, res) => {
    try {
      ...
      const stats = {
        ...
        availableFrom: competition.competitionAvailableTiming || competition.startTiming || competition.lastSaved,
  ...
  archiveCompetition: async (req, res) => {
    try {
      ...
      // Set as previous competition
      competition.previousCompetition = true;
      
      // If it's still live, set it to not live
      if (competition.isLive) {
      ...
      // Log the action
      console.log(`[${CURRENT_DATE_TIME}] Admin ${CURRENT_USER} archived competition: ${competition.competitionName} (ID: ${competition._id})`);
