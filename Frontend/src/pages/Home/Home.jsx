...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchStatistics = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/general/statistics');
  ...
  // Fetch testimonials/feedback from API
  useEffect(() => {
    const fetchTestimonials = async () => {
      try {
        setTestimonialsLoading(true);
        const response = await axios.get('http://localhost:5000/api/general/feedback?limit=3');

        if (response.data.success) {
          // Map feedback to testimonial format
          const mappedTestimonials = response.data.data.feedback.map((feedback, index) => ({
            id: feedback._id,
            name: feedback.name || `User ${index + 1}`,
            role: feedback.role || "Platform User",
            company: feedback.organization || "Code Quest",
            avatar: `/assets/images/testimonial${(index % 3) + 1}.jpg`,
        ...
        setTestimonials([
          {
            ...
            feedback: "Code-Quest transformed my coding journey. The platform's advanced features and interactive challenges helped me secure a top internship position.",
          },
          {
            id: 2,
            name: "Priya Patel",
            ...
            feedback: "The competitive challenges and performance analytics gave me valuable insights to improve my algorithms. My interview success rate improved dramatically.",
          },
          {
            id: 3,
            ...
            feedback: "As an educator, I find Code-Quest's assessment tools exceptional. The plagiarism detection and detailed analytics help me evaluate students effectively.",
          },
        ]);
      } finally {
  ...
  useEffect(() => {
    const fetchCompetitions = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:5000/api/general/competitions/all?limit=10');

        if (response.data.success) {
          const allCompetitions = response.data.data.competitions;

          // Filter and sort competitions by status
          const upcomingCompetitions = allCompetitions
            .filter(comp => comp.status === 'upcoming' || comp.status === 'scheduled')
            .slice(0, 2);

          const activeCompetitions = allCompetitions
            .filter(comp => comp.status === 'active' || comp.status === 'live' || comp.isLive)
            .slice(0, 1);

          const completedCompetitions = allCompetitions
            .filter(comp => comp.status === 'completed' || comp.status === 'ended')
            .slice(0, 1);
          ...
          const latestCompetitions = [
            ...
            date: new Date(comp.startTiming).toLocaleDateString('en-US', {
              month: 'long',
              day: 'numeric',
              year: 'numeric'
            }),
            participants: Math.floor(Math.random() * 2000) + 500, // Placeholder since not in API
            difficulty: comp.competitionType === 'beginner' ? 'Beginner' :
              comp.competitionType === 'intermediate' ? 'Intermediate' :
                comp.competitionType === 'advanced' ? 'Advanced' : 'Expert',
            prize: comp.competitionType === 'advanced' ? '$5,000' :
              comp.competitionType === 'intermediate' ? '$3,000' : '$1,500', // Placeholder
            status: comp.status === 'active' || comp.isLive ? 'Active' :
              comp.status === 'upcoming' || comp.status === 'scheduled' ? 'Upcoming' :
                comp.status === 'completed' || comp.status === 'ended' ? 'Completed' : 'Registration Open',
            id: comp._id,
            description: comp.competitionDescription,
            questionsCount: comp.questionsCount || 0,
        ...
        // Keep default competitions on error
        setCompetitions([
          {
  ...
  // Helper function to format numbers professionally without "+"
          ...
          >
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                ...
                {/* Enhanced Activity Stats - Simplified Large Numbers */}
                <MotionBox
                  variants={containerVariants}
                  initial="hidden"
                  animate="visible"
                ...
                >
                  {/* Main Stats Grid - Clean Number Display */}
                  <Grid container spacing={3} sx={{ maxWidth: '700px' }}>
                      ...
                      >
                        <Typography
                          ...
                          sx={{
                            ...
                            lineHeight: 1,
                            textShadow: isDark ? '0 0 20px rgba(188, 64, 55, 0.3)' : 'none',
                          }}
                        >
                          {statsLoading ? '...' : `${formatStatNumber(platformStats.totalStudents)}+`}
                        </Typography>
                    ...
                    <Grid item xs={6} sm={3}>
                      ...
                      >
                        ...
                        >
                          {statsLoading ? '...' : `${formatStatNumber(platformStats.totalTeachers)}+`}
                        </Typography>
                    ...
                    <Grid item xs={6} sm={3}>
                      ...
                      >
                        ...
                        >
                          {statsLoading ? '...' : `${formatStatNumber(platformStats.activeCompetitions)}+`}
                        </Typography>
                    ...
                    <Grid item xs={6} sm={3}>
                      ...
                      >
                        ...
                        >
                          {statsLoading ? '...' : `${formatStatNumber(platformStats.totalCompetitions)}+`}
                        </Typography>
            ...
            <Grid item xs={12} md={6}>
              <MotionBox
                ...
                sx={{
                  ...
                  transform: !isMobile
                    ? `perspective(1000px) 
                 rotateY(${(mousePosition.x / window.innerWidth) * 4 - 2
                    }deg) 
                 rotateX(${(-mousePosition.y / window.innerHeight) * 4 + 2
                    }deg)`
                    : "none",
      ...
      >
        ...
        <Container maxWidth="lg">
          ...
          >
            ...
            >
              {competitionsLoading ? "Loading Competitions..." : "Latest Competitions"}
            </Typography>
            ...
            >
              {competitionsLoading
                ? "Fetching the latest coding competitions..."
                : "Participate in our global coding competitions and challenge yourself against the best programmers worldwide."
              }
            </Typography>
          </MotionBox>
          ...
          {competitionsLoading ? (
            ...
            <Grid container spacing={4}>
              {competitions.map((competition, index) => (
                <Grid item xs={12} md={4} key={competition.id || index}>
                  <MotionCard
                    initial={{ opacity: 0, y: 30 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.5, delay: index * 0.1 }}
                  ...
                  >
                    ...
                    <CardContent sx={{ p: 3, flexGrow: 1 }}>
                      <Grid container spacing={2}>
                        ...
                        {competition.questionsCount > 0 && (
                          <Grid item xs={6}>
                            <Typography variant="body2" color="textSecondary">
                              Questions
                            </Typography>
                            <Typography variant="body1" fontWeight={600}>
                              {competition.questionsCount}
                            </Typography>
                          </Grid>
                        )}
                        <Grid item xs={competition.questionsCount > 0 ? 6 : 12}>
                          <Typography variant="body2" color="textSecondary">
                            Prize Pool
                          </Typography>
                          <Typography
      ...
      >
        ...
        <Box
          sx={{
            ...
            zIndex: -1,
            opacity: 0.03,
            backgroundImage: `url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M11 18c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm48 25c3.866 0 7-3.134 7-7s-3.134-7-7-7-7 3.134-7 7 3.134 7 7 7zm-43-7c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm63 31c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zM34 90c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z' fill='${isDark ? "%23ffffff" : "%23000000"
        ...
        <Container maxWidth="lg">
          ...
          >
            ...
            >
              {testimonialsLoading ? "Loading Testimonials..." : "User Testimonials"}
            </Typography>
            ...
            >
              {testimonialsLoading
                ? "Fetching user feedback and experiences..."
                : "Hear from our users about how our platform has transformed their coding and assessment experience. Real stories from real developers who've taken their skills to the next level."
              }
            </Typography>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <Chip
                          size="small"
                          label={testimonial.company || "Code-Quest Platform"}
                          sx={{
                            backgroundColor: isDark
                              ? "rgba(255,255,255,0.05)"
                              : "rgba(0,0,0,0.04)",
