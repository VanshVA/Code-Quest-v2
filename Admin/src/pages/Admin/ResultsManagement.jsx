...
function ResultsManagement() {
  ...
  const fetchSubmissions = async (competitionId) => {
    try {
      ...
      if (response.data.success) {
        ...
        // Get the full competition details
        ...
        if (compResponse.data.success) {
          setSelectedCompetition({
            ...response.data.data.competition,
            ...compResponse.data.data.competition
          });
        }
      } else {
  ...
  const filteredSubmissions = submissions.filter(submission => {
    const matchesSearch = submission.student.name.toLowerCase().includes(submissionSearchTerm.toLowerCase()) ||
                         submission.student.email.toLowerCase().includes(submissionSearchTerm.toLowerCase());
    
    if (submissionStatusFilter === 'all') return matchesSearch;
    if (submissionStatusFilter === 'complete') return matchesSearch && submission.answeredAll;
    if (submissionStatusFilter === 'incomplete') return matchesSearch && !submission.answeredAll;
  ...
  const handleExportSubmissions = () => {
    ...
    try {
      // Create CSV header
      let csvContent = "Submission ID,Student Name,Student Email,Submission Time,Questions,Answers,Completed\n";
      
      // Add each submission as a row
      submissions.forEach(submission => {
        csvContent += `${submission.submissionId},${submission.student.name},${submission.student.email},${submission.submissionTime},${submission.questions},${submission.answers},${submission.answeredAll ? 'Yes' : 'No'}\n`;
      });
          ...
          >
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              >
                ...
                <MenuItem selected={statusFilter === 'active'} onClick={() => handleStatusFilterChange('active')}>
                  Active
                </MenuItem>
                <MenuItem selected={statusFilter === 'ended'} onClick={() => handleStatusFilterChange('ended')}>
                  Ended
                </MenuItem>
                ...
                <TableBody>
                  {competitions.length > 0 ? (
                    competitions.map((competition) => {
                      const isExpired = competition.status === 'ended' || new Date(competition.endTiming) < new Date();
                      return (
                        <TableRow 
                          key={competition._id}
                          hover
              ...
              >
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                  <Box>
                    ...
                    <Chip
                      label={selectedCompetition.type}
                      color={
                        selectedCompetition.type === 'TEXT' ? 'primary' :
                        selectedCompetition.type === 'MCQ' ? 'secondary' : 
                        'warning'
                      }
                      size="small"
                    ...
                    <Grid item xs={12} sm={6}>
                      ...
                      >
                        <Typography variant="h3" fontWeight="bold" color="success.main">
                          {submissionStats.completedSubmissions}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">Completed Submissions</Typography>
                      </Paper>
                    </Grid>
                    <Grid item xs={12}>
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                          ...
                          <Typography variant="body2">
                            {submissionStats.totalSubmissions > 0 
                              ? Math.round((submissionStats.completedSubmissions / submissionStats.totalSubmissions) * 100) 
                              : 0}%
                          </Typography>
                        </Box>
                        <LinearProgress
                          ...
                          value={submissionStats.totalSubmissions > 0 
                            ? (submissionStats.completedSubmissions / submissionStats.totalSubmissions) * 100
                            : 0}
          ...
          >
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              ...
              >
                ...
                <MenuItem selected={submissionStatusFilter === 'complete'} onClick={() => handleSubmissionStatusFilterChange('complete')}>
                  Complete Submissions
                </MenuItem>
                <MenuItem selected={submissionStatusFilter === 'incomplete'} onClick={() => handleSubmissionStatusFilterChange('incomplete')}>
              ...
              <Button
                variant="outlined"
                startIcon={<Refresh />}
                onClick={() => fetchSubmissions(selectedCompetition.id)}
                size="medium"
                sx={{ 
                  borderRadius: '8px',
                  textTransform: 'none',
                  ...
                  {filteredSubmissions.length > 0 ? (
                    filteredSubmissions
                      ...
                      .map((submission) => (
                        ...
                        >
                          <TableCell>{submission.submissionId.substring(0, 8)}...</TableCell>
                          <TableCell>{submission.student.name}</TableCell>
                          <TableCell>{submission.student.email}</TableCell>
                          <TableCell>{formatDate(submission.submissionTime)}</TableCell>
                          <TableCell>{submission.questions}</TableCell>
                          <TableCell>{submission.answers}</TableCell>
                          <TableCell>
                            <Chip
                              label={submission.answeredAll ? 'Complete' : 'Incomplete'}
                              color={submission.answeredAll ? 'success' : 'warning'}
                              size="small"
                              sx={{ borderRadius: '4px' }}
                            />
                          </TableCell>
